...
class ProceduralIntersectionApp : public VulkanRayTracingApp
{
    ...
    magma::AccelerationStructureAabbs aabb;
...
public:
    ...
    {
        alignas(16) VkAabbPositionsKHR positions = {
            -1, -1, -1,
            1, 1, 1
        };
        aabbBuffer = utilities::makeInputBuffer(positions, cmdBufferCopy, allocator);
        aabb = magma::AccelerationStructureAabbs(aabbBuffer.get());
    }

    void createAccelerationStructures()
    {
        bottomLevel = std::make_unique<magma::BottomLevelAccelerationStructure>(device,
            std::list<magma::AccelerationStructureGeometry>{aabb},
    ...
    void buildAccelerationStructures()
    {
        ...
        cmdCompute->begin(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
        {
            ...
            cmdCompute->buildAccelerationStructure(bottomLevel, {aabb}, scratchBuffer);
            cmdCompute->pipelineBarrier(
                VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR,
                VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR,
